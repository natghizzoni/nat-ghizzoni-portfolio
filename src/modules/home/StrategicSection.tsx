...
const valuePropIcons = [
  <svg key="0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-6 h-6 md:w-7 md:h-7">
    <path strokeLinecap="round" strokeLinejoin="round" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
  </svg>,
  <svg key="1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-6 h-6 md:w-7 md:h-7">
    <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
  </svg>,
  <svg key="2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-6 h-6 md:w-7 md:h-7">
      ...
      <div className="max-w-[1600px] mx-auto w-full">
        ...
        <div className="flex flex-col gap-3 md:gap-4 mb-8 md:mb-6">
          ...
          <h2 className="font-['Inter'] text-[24px] leading-[32px] md:text-[32px] md:leading-[44px] text-[#1a1433]">
            <span className="font-bold">{s.titleBold}</span>
            <span className="font-normal">{s.titleLight}</span>
          </h2>
          <p className="text-[#5c6378] text-[14px] md:text-[16px]">
            <span className="font-bold text-[#101828]">{s.shortTimeBold}</span>
            <span>{s.shortTimeText}</span>
          </p>
        </div>
            ...
            >
              ...
              >
                ...
                <div className="relative z-10 flex items-center justify-between gap-3">
                  <p className="font-normal text-[13px] md:text-[16px] leading-6 uppercase tracking-[0.8px] text-white/80">
                    {s.experienceLabel}
                  </p>
                  <button
                    onClick={(e) => { e.stopPropagation(); setOpen(true); }}
                    className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 active:bg-white/30 transition-colors px-3 py-1.5 rounded-full text-white/70 hover:text-white text-xs font-medium shrink-0"
                  >
                    {s.seeDetail}
                ...
                {/* Industry tags — sorted longest→shortest so they wrap into balanced rows */}
                <div className="relative z-10 flex flex-wrap gap-2 md:gap-3 items-center">
                  {[...s.tags].sort((a, b) => b.length - a.length).map((r) => (
                    <span
                      key={r}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-[10px]">
                  {s.breakdown.map(({ suit, value, label }) => (
                    <div
                      key={suit + value}
                      className="flex flex-col items-center"
                      style={{
                        padding: "4px 14px 14px",
                        background: "rgba(87,75,224,0.4)",
                        border: "1px solid rgba(155,148,232,0.2)",
                        borderRadius: "12px",
                      }}
                    >
                      <span style={{ fontSize: "18px", color: "rgba(155,148,232,0.5)", marginTop: 0 }}>
                        {suit}
                      </span>
                      <p
                        style={{
        ...
        <div className="flex flex-row flex-wrap items-center gap-x-8 gap-y-4 pt-8 md:pt-[24px] mt-6 md:mt-0">
          <p className="text-[#5c6378] text-[14px] md:text-[16px] flex-[1_1_260px] min-w-0">
            <span className="font-bold text-[#101828]">{s.lookingForBold}</span>
            <span>{s.lookingForText}</span>
          </p>
          <Link
            href="/cv"
